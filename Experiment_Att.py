...
def experiment(df, filename = 'metadata/phys.edges', nodeAttributeFile = None, multipleGraph = False, is_perm = False, 
    has_noise = False, GraphType = 'Directed', bandNumber = 2, adaptiveLSH = False, LSHType = 'Euclidean', loop_num = 3):
    """
    ...
    for i in range(loop_num):
        ...
        for index in range(1, len(list(attributes)) + 1):
            ...
            if len(sub_attributes) >= bandNumber:
                ...
                pairs_count_att.append(len(pair_count_dict)/float(matching_matrix.shape[0]*matching_matrix.shape[1]))      
    ...
    rank_att_score = rank_att_score / float(loop_num)
    correct_att_score = correct_att_score / float(loop_num)
    pairs_computed = pairs_computed / float(loop_num)

    print rank_att_score
    print correct_att_score
    print pairs_computed
