...
class PureNetalign:
	def __init__(self, fname):
		self.fname = fname
	def sim_netalign(self, df, filename, LSHType):
		...
		for center_id in centers:
			for g in graph_attrs.keys():
				if (center_id, g) not in sim_matrix and (g, center_id) not in sim_matrix and g != center_id:
					...
					matching_matrix[(center_id, g)] = self.filter_sim_to_match(sim_matrix[(center_id, g)], 0.2)	
	...
	def run(self, filename = 'facebook', LSHType = 'Cosine'):
		if os.path.isfile(self.fname+'.pkl'):
			...
			df = pd.DataFrame()
		df = self.sim_netalign(df, filename = filename, LSHType=LSHType)
