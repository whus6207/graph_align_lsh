...
if __name__ == '__main__':
	...
	cos_num_planes = {'facebook':[25], 'dblp-A':[40,100]}
	thresholds = [0.2]
	LSHs = ['Cosine']
	band_numbers = {'facebook':[4], 'dblp-A':[1,2]}
	...
	for f in folders:
		for noise_level in noise_levels:
			...
			for lsh in LSHs:
				netalign_runner.run(f,lsh)

			preprocessing(edge_dir = 'Data/'+f+'.edges', save_dir = f, number = num_graph-1, noise_level = noise_level
				, node_dir = 'Data/'+f+'.nodes')
			hashalign_runner = HashAlign(fname)
			hashalign_runner.run(folders = [f], band_numbers = band_numbers[f], cos_num_plane = cos_num_planes[f], LSHs = LSHs, thresholds = thresholds)
