...
def experiment(df, filename = 'Data/facebook.edges', nodeAttributeFile = None,  
	...
	# Experiment on different attributes

	exp_att = [['Degree', 'EgonetDegree', 'AvgNeighborDeg', 'EgonetConnectivity', 'PageRank'],
	['Degree', 'NodeBetweennessCentrality', 'FarnessCentrality', 'PageRank', 'NodeEccentricity'\
	,'EgonetDegree', 'AvgNeighborDeg', 'EgonetConnectivity']]
	origin = {}
	for g, att in graph_attrs.iteritems():
		origin[g] = att

	for attr in exp_att:
		attributes = attr
		for g in graph_attrs.keys():
			graph_attrs[g] = origin[g][['Graph', 'Id'] + attr]
...
if __name__ == '__main__':
	...
	if os.path.isfile(fname):
		...
		df = pd.DataFrame(
			columns=['filename','nodeAttributeFile', 'attribute_num', 'has_noise', 'GraphType'\
	...
	for lsh in LSH:
		df = experiment(df, filename = 'Data/facebook.edges', nodeAttributeFile = None, 
				...
				adaptiveLSH = False, LSHType = lsh, noise_level = 0.01,
				center_distance = 'canberra', find_center = 0)
		

	pickle.dump(df, open(fname,'wb'))

	writer = pd.ExcelWriter('exp_result_att_num.xlsx')
